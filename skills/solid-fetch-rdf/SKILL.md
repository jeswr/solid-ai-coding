...
| `n3.Writer` does NOT escape IRIs or strip control chars when serialising untrusted RDF | When you serialise a term value that came from **untrusted input** (a parsed foreign RDF `@id`, a JSON/HTTP field, a user-supplied URL) through `n3.Writer`, the value is emitted **verbatim** between `<…>`. A value containing a Turtle-`IRIREF`-forbidden char (`>` `<` `"` `{` `}` `|` `^` backtick `\` space or a C0 control) **breaks out of the angle brackets** and can inject forged triples — one intended quad can serialise into two, the second attacker-chosen (verified repro: [rdfjs/N3.js#659](https://github.com/rdfjs/N3.js/issues/659)). A boolean `new URL()`/`isHttpIri` check is **not** sufficient: it validates the string but forwards the raw value, so the breakout chars survive. Filter every IRI through **[`@jeswr/rdf-serialize`](https://github.com/jeswr/rdf-serialize)** before building the `NamedNode`: use **`safeHttpIri`** for a field that must be http(s) — it percent-encodes the forbidden set **and drops non-http(s) schemes** (returns `undefined`, so the caller omits the triple); use **`escapeIri`** for a scheme-agnostic subject/object (a legitimate `urn:`/`did:` id) — it only percent-encodes the forbidden set and does **not** enforce a scheme, so don't rely on it for scheme filtering. Both are lexical (RDF identity is lexical: they preserve the value byte-for-byte apart from the escaped chars). The same `Writer` also emits raw `ESC`/`DEL`/C1 controls (`U+001A`,`U+001B`,`U+001F`,`U+007F`,`U+0080`–`U+009F`) inside literals — strip those from untrusted text bodies before writing. (~40 cumulative adversarial rounds across ~20 `@jeswr` libs re-derived this before it was consolidated; don't hand-roll copy N+1.) |
